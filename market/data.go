...
func Get(symbol string) (*Data, error) {
	...
	if isXyzAsset {
		...
		// Use configured API (CoinAnk or OKX) for regular crypto assets
		klines3m, err = getKlines(symbol, "3m", 100)
		if err != nil {
	...
	if isXyzAsset {
		...
		klines4h, err = getKlines(symbol, "4h", 100)
		if err != nil {
...
func GetWithTimeframes(symbol string, timeframes []string, primaryTimeframe string, count int, emaPeriods, rsiPeriods, atrPeriods []int) (*Data, error) {
	...
	for _, tf := range timeframes {
		...
		if isXyzAsset {
			...
			// Use configured API (CoinAnk or OKX) for regular crypto assets
			klines, err = getKlines(symbol, tf, 200)
			if err != nil {
